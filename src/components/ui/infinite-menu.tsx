...
function createProgram(gl: WebGL2RenderingContext, shaderSources: [string, string], transformFeedbackVaryings: string[] | null, attribLocations: Record<string, number> | null) {
...
class ArcballControl {
    ...
    constructor(canvas: HTMLElement, updateCallback?: (deltaTime: number) => void) {
        this.canvas = canvas;
...
class InfiniteGridMenu {
    ...
    camera = {
        ...
        fov: Math.PI / 4,
        aspect: 1,
        position: vec3.fromValues(0, 0, 3) as vec3, // Added explicit type cast
    ...
    #initTexture() {
        ...
        Promise.all(
            this.items.map(
                item =>
                    new Promise((resolve) => {
                        ...
                        img.onerror = () => {
                            // Return a placebo image in case of error to prevent halting
            ...
            images.forEach((img: any, i) => {
                const cx = (i % this.atlasSize) * cellSize;
                const cy = Math.floor(i / this.atlasSize) * cellSize;

                const scale = Math.max(cellSize / img.width, cellSize / img.height);
                const w = img.width * scale;
                const h = img.height * scale;
                const x = (cellSize - w) / 2;
                const y = (cellSize - h) / 2;

                ctx.save();
                ctx.beginPath();
                ctx.rect(cx, cy, cellSize, cellSize);
                ctx.clip();
                ctx.drawImage(img, cx + x, cy + y, w, h);
                ctx.restore();
            });
